(ns pension.operation
  "OperationActor -- one pension-funding operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Pension-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Pension Governor (:govern) and the rollout
  ...
  One graph run = one pension-funding operation (intake -> advise ->
  ...
  decision to a human operator (the licensed pension administrator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:disbursement/pay`/`:payout/continue` ALWAYS reach this
  node when the governor is clean -- see `pension.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [pension.pensionllm :as pensionllm]
...
(defn build
  ...
  `pension.store/Store`).
  opts:
    :advisor      -- a `pension.pensionllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (pensionllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Pension-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (pensionllm/-advise advisor store request)]
            {:proposal p :audit [(pensionllm/trace request p)]})))

      ;; Pension Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
