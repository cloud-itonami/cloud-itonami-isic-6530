...
(def confidence-floor
  ...
  `pension.kernels.gate/confidence-floor-x100` (integer x100 in the
  safety kernel); this def is kept for callers/docs and pinned equal by
  `pension.kernels.gate-test`."
  0.6)
...
(defn check
  ...
  (let [spec-v     (spec-basis-violations request proposal st)
        ...
        unvested-v (member-not-vested-violations request st)
        missing-v  (disbursement-missing-violations request st)
        exceeds-v  (disbursement-exceeds-entitlement-violations request st)
        payout-v   (member-not-in-payout-violations request st)
        pol-v      (proof-of-life-failed-violations request proposal st)
        double-v   (double-payment-violations request st)
        hard (into [] (concat spec-v evid-v unvested-v missing-v exceeds-v
                              payout-v pol-v double-v))
        conf (:confidence proposal 0.0)
        stakes? (boolean (high-stakes (:stake proposal)))
        ;; The decision itself is delegated to the safety kernel
        ;; (pension.kernels.gate, integer-coded fail-closed core);
